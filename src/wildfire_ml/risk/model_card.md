...
### Sprint 6-C Notu — WorldCover gerçek raster (2026-05-17)
Sprint 6-C (2026-05-17): `attach_real_features` WorldCover one-hot sentetik iskeletten **gerçek ESA WorldCover v200 raster** örneklemeye geçirildi (`build_real_dataset.py`; ESA kod 10/20/30/40/50/60/90/100 → 8 one-hot sınıf; 70/80/95 Beynam'da yok → all-zero, 432/220008 ≈ %0.2 eşleşmeyen). **Metrik etkisi nötr/marjinal (dürüst değerlendirme):** spatial-block CV (leakage-safe, asıl metrik) ROC-AUC b1 0.819±0.037 → 6-C **0.820±0.038** (+0.001, gürültü içinde); holdout test ROC-AUC 0.858 → 0.865, Recall 0.761 → 0.781 (hafif pozitif ama test, leakage-safe CV'den az güvenilir). **Sebep:** Beynam neredeyse homojen orman → land-cover varyansı düşük; 10km FIRMS haversine label yarıçapı 250m land-cover detayını yıkar; yangın sinyali topo (DEM) + meteo (Open-Meteo) + fire-history'den geliyor, land-cover'dan değil. Gerçek veri yine de doğru yön (dürüst model; heterojen bölgede land-cover ayırt edici olabilir). **Paket sürümü b1 korundu** — anlamlı performans değişimi yok, sürüm bumpı haklı değil. Backlog: çok-kaynak FIRMS (`VIIRS_NOAA20_SP`), `export_risk` ruff (I001/N806), `_retry_get` 4xx gereksiz-retry.
...
### Sprint 6-C Note — real WorldCover raster (2026-05-17)
Sprint 6-C (2026-05-17): `attach_real_features` WorldCover one-hot switched from synthetic skeleton to **real ESA WorldCover v200 raster** sampling (`build_real_dataset.py`; ESA codes 10/20/30/40/50/60/90/100 → 8 one-hot classes; 70/80/95 absent in Beynam → all-zero, 432/220008 ≈ 0.2% unmapped). **Metric effect neutral/marginal (honest assessment):** spatial-block CV (leakage-safe, primary metric) ROC-AUC b1 0.819±0.037 → 6-C **0.820±0.038** (+0.001, within noise); holdout test ROC-AUC 0.858 → 0.865, Recall 0.761 → 0.781 (slightly positive but holdout is less reliable than leakage-safe CV). **Reason:** Beynam is nearly homogeneous forest → low land-cover variance; the 10km FIRMS haversine label radius washes out 250m land-cover detail; the fire signal comes from topography (DEM) + meteo (Open-Meteo) + fire-history, not land-cover. Real data is still the correct direction (honest model; land-cover may discriminate in heterogeneous regions). **Package version kept at b1** — no significant performance change, a version bump is not warranted. Backlog: multi-source FIRMS (`VIIRS_NOAA20_SP`), `export_risk` ruff (I001/N806), `_retry_get` 4xx needless-retry.
